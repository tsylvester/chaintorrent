...
* Two nodes ingesting the same unlisted package produce different ciphertext. This is resolved at the identity layer, not the payload layer: **State-Locked Escrow Registration** means the first transaction to land wins, and the losing node destroys its local ciphertext, header sidecar, master scalar, expanded cipher state, and buffered keystream, then resolves the winner's authenticated deployment descriptor and transport locator set.
...
* **Retention here is the user's decision, not the protocol's.** It is their disk. The client defaults to unbounded and exposes a quota; where a quota forces eviction, artifacts currently linked into a live `node_modules` are pinned, and the client warns before evicting anything the identity could not re-authorize. Eviction is not an ordinary cache miss — re-materializing plaintext requires a fresh decryption attempt under a current state view, which fails if the entitlement has since transferred, so silent eviction can destroy access the user reasonably believed was permanent.

## Entitlement Ledger and Escrow Contract

Deployed on Ethereum: the ecosystem is chosen, because ERC-721 entitlements and Solidity contracts are its native objects; which network — an L2, where delivery verification on BN254 is cheap, or mainnet, whose BLS12-381 precompiles remove pairings from verification — is held in the To-Do list. First Finder entitlements are priced at $0.00; the explicit-publisher dogfood asset under Transaction Flow Proof is the sole nonzero-price exception.
...
* **Abuse controls.** Payment locks carry a short, published expiry, so a seller who never delivers ties up a buyer's funds only briefly; a listing bond is an optional market-design addition. Zero-price grants and relayer-paid mints are rate-limited per identity by relayer policy, because each verifies a proof on-chain and an unmetered path converts the grant pool into gas. The claim verifier's attestation key is registered on-chain with rotation and revocation, since its compromise would hand publisher authority over escrowed assets to an attacker. The per-attempt state view is read from more than one configured node, so one censoring or inconsistent node cannot deny reads.
...
**The salt's custodian is an open decision, held in the To-Do list.** It was previously sited with the provisioning layer, which no longer exists. The candidates are the First Finder, which already holds the escrow master scalar but must not become a dependency at claim; the claim verifier, which already authenticates the claimant but would make a lost salt a permanent-failure mode; or dropping the on-chain commitment, with the verifier establishing the claim set from upstream metadata at verification time, which publishes no maintainer linkage at all and leaves nothing to lose. The claim-set mechanism above is unchanged by whichever is chosen.
...
**`Secp256k1Adapter` is shipped for the chain layer**, because Ethereum is the launch ecosystem and an Ethereum account *is* a secp256k1 keypair, permitting no alternative there. That is a constraint the target imposes, not a protocol preference — the specific network is not chosen, and the scheme a chain layer uses follows from whichever chain adapter resolves. A non-EVM target ships its own signature adapter and changes nothing above the chain layer.
...
Credential delivery is exercised at every mint and every transfer, with the proof verified on-chain each time; that is where the cryptography is proven rather than at read time. Stake-weighted selection, Sybil resistance, retention assignment, and audit consequences remain deferred, but content discovery, seeding, leeching, entitlement, verified delivery, and local decryption must function end to end.
