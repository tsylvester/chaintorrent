...
**Timing.** Pairing precompiles exist on every EVM chain, and BLS12-381 precompiles are specified for mainnet and must be confirmed per launch network before they are relied on. The software supply chain is converging on provenance attestations and transparency logs, which the protocol consumes as publisher proofs. And the project's own research has just closed the problem that made the design impossible a week earlier.
...
**For the ecosystem.** Provenance attestations recorded on chain. Advisories that follow the plaintext fingerprint across repackaging, so a flag cannot be shed by re-uploading under a new name. A public dependency inventory that makes known-vulnerable versions visible and makes a paid product's consumption of open-source packages visible. The same inventory is a privacy exposure for individuals, and the case carries it under Risks rather than presenting only this half.
...
The [risk register](risk-register.md) holds twenty risks with impact, likelihood, affected components, and signals. This section carries the ones that bear on the decision to proceed, at the register's ratings.

**Delivery breadth, High impact, High likelihood.** One hundred fifteen operational requirements each with a boundary-crossing proof, twenty-seven acceptance scenarios on clean machines, four control surfaces, signed installation and service lifecycle on every supported platform, a contract suite, three services, and a demonstration harness that the completion boundary makes load-bearing. No code, team, timeline, or budget exists. Mitigation: the harness phase is mapped at ticket resolution and nothing beyond it is; its measured throughput is the first calibration; the supported platform matrix is decided explicitly before the onboarding phase; a demonstrable milestone at the package host produces something to show and measure before delivery, publishing, and claims.
...
**Blocking selections, Medium impact, High likelihood.** Custody, launch network and curve, escrow-salt custodian, attempt-rule parameters and piece-group size, license text, recovery UX. Mitigation: the technical approach records a default for each; custody defaults to a local daemon-held keystore under OS credential protection as the first adapter; the salt custodian defaults to dropping the on-chain commitment, the only candidate with no lost-salt failure mode that satisfies the rule that claim verification depend on no unavailable party; the harness runs on BN254 until the network is chosen from measured gas.

**Adapter registry governance and scaffolding, Medium impact, Medium likelihood.** Adapters are resolved from a governance-controlled on-chain registry, immutable once bound, and no document says who governs it. The relayer, project seed host, and claim verifier are centralized conveniences the project operates. Mitigation: record the registry's governance as a workplan item; ship default configuration with more than one discovery source and more than one state-read node; keep every service behind an interface with a documented replacement path.
...
- The engineering surface is wide and platform breadth is the largest delivery risk; no document lists the supported platforms.
...
- Six blocking selections remain open, two with no determinant in the sources.
...
- **Build the cryptographic validation harness.** Thirty-two tickets at the dependency map's resolution, four independent starting points, closing at the report node that records capsule, envelope, and proof sizes, decapsulation time per piece group, proof generation and verification time, and delivery gas for a mint and a transfer on both curves, and that carries the phase's integration test and commit. The Solidity verifier is production code. Owner: cryptography implementer. Stop criterion: none at this step; the harness produces the inputs to the next.
...
- **Resolve the measurement-gated selections from the harness output**: launch network and curve, piece-group size, attempt-rule parameters. Re-run the whole-target evaluation with measured costs. Owner: project lead with the cryptography implementer.
- **Re-map the protocol core phase to ticket resolution from what the harness taught**, including its measured throughput, and produce the first evidence-based estimate of timeline and engineering cost. Owner: project lead. Stop criterion: if the estimate is beyond what the project can fund, that is decided here, before the wide work begins.
- **Build the protocol core and contract suite, with the swarm sprint in parallel.** Domain model, payload cipher and commitments, signature services, registry and entitlement contracts calling the harness's verifier, chain and settlement adapters, adapter registry and factory; swarm transport, seed host, and ciphertext store alongside, since they need no chain. Owner: implementers by family.
- **Build the local daemon and package serving to the demonstrable milestone.** Durable jobs, plaintext CAS and resolution order, First Finder ingest, package host; identity and custody early because the installer needs it. At the package host, an ordinary `npm install` resolves against the swarm. Measure the north star and the latency guardrail on the dogfood population here. Owner: daemon implementer. Stop criterion: the guardrail; if the benefit is not felt on the dogfood population, the case does not proceed to the adopting population.
- **Build credential delivery and per-attempt authorization**, closing the read path: mint delivery through the relayer, local decryption under the attempt rule, interval-end destruction.
...
- [MVP Application Requirements.md](../research/MVP%20Application%20Requirements.md), one hundred fifteen operational requirements and twenty-seven acceptance scenarios.
...
- [cryptography-requirements.md](../research/cryptography-requirements.md), fifteen requirements and fifteen preferences.
...
No source supports a figure and none is invented. What the plan produces, and when: delivery-verification gas on both curves at the harness, which is the largest input to the relayer's per-install cost and to the network choice; relayer gas per install and per identity binding, and swarm bandwidth per install, at the first dogfood acquisition; engineering throughput, tickets closed per unit of time under the repository's discipline, across the harness phase, which is the calibration for the first timeline and cost estimate; and external review and legal costs, which have market rates and can be quoted now. Spend categories are engineering across applied cryptography, Rust systems, Solidity, platform packaging, and Tauri; external cryptographic review; the relayer grant pool, sized from measured gas; infrastructure for the seed host, site, and test chain; and legal. The decision point for funding the wide work is after the harness, when the protocol core phase is re-mapped to tickets with a measured throughput behind it.
...
The technical approach and the dependency map hold the build order. The groupings, addressed by role and not by ordinal, are the cryptographic validation harness, at ticket resolution; the protocol core and contract suite, with the swarm sprint running in parallel because it needs no chain; the local daemon and package serving, reaching the demonstrable milestone; the onboarding shells and services; and acceptance and release. Resolution decays with distance: when the harness phase closes, the next phase is re-mapped to tickets from what was learned, and the decay shifts outward. The original case's numbered phases and their ordering of contracts before swarm are superseded by this statement.
...
## Corrections to the original case, recorded once

The research requirements number fifteen, not fourteen; the whole-target evaluation covered the first fourteen because the fifteenth, which governs how conclusions are written, was added afterward. BLS12-381 precompile availability is specified for mainnet and must be confirmed per launch network, not stated as settled. The Application Requirements contain one hundred fifteen operational requirements and twenty-seven acceptance scenarios. The delivery phases are addressed by role. The research involved two maintainers, an independent re-derivation agent, and a third agent whose proposals were reviewed and excluded. The claims drawn from outside the repository are now labelled wherever they appear.
