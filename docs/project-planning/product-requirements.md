...
**The state of the work.** The cryptographic problem that made the design impossible is closed at the research level and adopted into the specifications. Technical feasibility is high; every component is conventional. Delivery feasibility is undetermined: no code, team, timeline, or budget exists, and the surface is wide. The validation harness is designed to calibrate that. The launch network, curve, platform matrix, custody and its in-place upgrade, the multi-device model, the identity's chain-level form, the escrow record's form, and the element mapping to Rust and Solidity are decided; the attempt-rule parameters and piece-group size are measured by the harness; the license text and custody recovery UX have their milestones. Compliance items on the release path are unstarted.
...
- **Identity, wallet, and custody.** Separable capabilities bound to one principal; declared custody capabilities; one identity, one relayer-paid binding, one envelope-key registration; the holder seed held on root devices only, every other device admitted under its own device key as a signer on the identity's contract account and revoked there; custody blobs tagged by adapter version and upgraded in place by a verified migration; recovery without silent principal change; EIP-1193, WalletConnect, EIP-712; local default identity; envelope keys never wallet keys.
...
| Contract identity form | A contract account verified by ERC-1271 whose signer set is the identity's device registry, with a signed intent on every identity-bound mutation | An externally owned account as the identity; a `msg.sender` model | Device admission and revocation live in state the attempt rule reads; signed intents keep paymaster-sponsored, relayer-submitted, and self-funded calls on one entry point, so the sponsorship choice waits for the relayer milestone |
| Seed placement | The holder seed on root devices the user chooses; every other device admitted under its own key | The seed on every device | A device added for convenience never becomes a custodian of the identity, and a lost device is closed by revoking its signer |
...
- **Default key custody** is a local keystore under the OS credential store, holding the default identity and the envelope keys as the first `IKeyCustodyAdapter` with keys derived from a holder seed; external wallets sign only, through EIP-1193 or WalletConnect; Frame is evaluated as the first desktop provider. No operating system provides an Ethereum wallet, and no Ethereum wallet can hold envelope keys, master scalars, or publisher seeds, so a custody adapter exists regardless and the wallet covers only the chain-signing key. Every custody blob carries a header naming the adapter and its version, the wrapping-key identifier, and the derivation parameters, and a reader selects its unwrap path from the header, as Chromium's versioned credential encryption and Tink's keysets do. An adapter upgrades in place as a durable checkpointed job: unwrap under the old adapter into the secret region, wrap under the new one to a temporary blob, rename it into place beside the old, then verify that the handshake key and envelope public keys re-derived from the rewrapped seed match the on-chain binding and envelope-key registration and that a held envelope decrypts to a valid credential; only then are the old blob and its wrapping key retired, and a mismatch rolls back. The seed is the invariant, so an upgrade touches nothing on chain.
...
- **Crate and path layout**: one workspace with a domain crate, a workflows crate, a crate per adapter family, and a crate per deployable; each crate is created in the node of the first module that lives in it, never framed ahead of need; the workspace manifest lists its members by glob, so adding a crate leaves the manifest unchanged; one function per file; crate directories hyphenated, module directories underscored; a ticket is named `crate/module`.
- **Workspace lints**: the workspace manifest's lint table forbids `unsafe_code` and denies clippy's `unwrap_used`, `expect_used`, `panic`, and `as_conversions` in production code; test modules and code behind the `mocks` feature are exempt.
- **Shared test support**: one dev-only crate, `test-support`, provides `assert_same` and every other test-only item the standards topics name; every crate takes it as a dev-dependency and imports it as `test_support::assert_same`, and no crate carries its own copy.
- **Canonical encoding** is a pair of adapters behind separate repo-owned interfaces, `IEncoderAdapter` and `IDecoderAdapter`, which declare one shared versioned encoding identifier, so a later encoding is a further pair of implementations and a deployment's hash-card names the one it uses. The MVP implementations, `AbiEncoderAdapter` and `AbiDecoderAdapter`, use Ethereum ABI encoding through `alloy`'s sol types, which the contracts reproduce natively for every value they recompute; the encoding adapters own the sol-type mirrors and the conversions to and from every type they encode, so no other crate depends on `alloy` for encoding. The one encoding serves everything hashed, signed, stored, or framed over IPC. The encoder and the decoder are separate modules of the encoding adapter crate; the decoder takes untrusted bytes and returns the owned type or its error.
- **Type ownership**: a type is owned by, and lives in, the module that implements it; the `domain` crate holds only the types its own modules implement.
- **Tools and dependencies arrive with their first consumer**: a dependency is pinned, a tool configured, and a continuous-integration step added by the ticket that first needs it, never ahead of it. Code-signing enrollment is external work with its own lead time and starts with the harness grouping.
- **Secret-typed values** are a `domain` type holding every secret the protocol handles, master scalars, credentials, piece-group keys, envelope secrets, and seeds, that implements no formatting or serialization trait, exposes the wrapped value only through an explicit accessor to the cryptographic operations and custody wrapping that consume it, and zeroizes on drop; that it cannot be formatted is proven at compile time. Telemetry owns per-request correlation and tracing setup and does no redaction of its own.
...
- **The identity's chain-level form is a contract account** verified by ERC-1271, whose signer set is the identity's device registry: each admitted device's key is a signer with its own permissions, admission and revocation are signed intents under the control branch, and the per-attempt wallet-control assertion is satisfied only by a key the account currently admits, read in the same state view as the authorization. Every contract function that mutates identity-bound state takes the acting identity and a signed intent, and the contracts enforce lock expiry and no volume limit, so paymaster-sponsored, relayer-submitted, and self-funded calls share one entry point. The sponsorship mechanism is chosen at the relayer milestone and forces no contract rework. The harness measures the verifier's cost; sponsorship overhead is measured at the relayer milestone.
- **A pre-install web account** owns onboarding state, preferences, a roster of linked identities and devices, and optionally an end-to-end encrypted recovery backup of the holder seed under a user-held secret; it never owns an entitlement. The principal is the identity, never the account; a sign-in provider cannot be translated into a key; an installation links to an account by a signed challenge proving possession of the identity's own keys; the mapping is held only by the account service, never on chain, and is optional. The website account, rendezvous, remote head, and hosted instance are deferred to V2 with architectural protection recorded in MVP Scope; the MVP site carries the education tier with a WebAssembly demonstration built from the client's crates and reserves the link step in the installer's consent flow.
- **Multiple devices share one identity without moving the seed**, after Matrix cross-signing and Keybase sibling keys. The holder seed lives only on root devices the user chooses and derives domain-separated branches: the control branch, holding the handshake key and authority over binding, rotation, and the device set, which never leaves a root; and the envelope branch, holding the envelope secrets, which any reading device may receive. Every other device generates its own device key, pairs with a root by QR code or short authentication string, is admitted as a signer on the identity's contract account with per-device permissions, and receives a delegation signed by the handshake key naming its role; a reading role receives the envelope secrets over the paired channel, never the seed. A further root is added only by a deliberate seed transfer the user initiates. Roles are versioned capabilities the custody adapter declares, so later roles arrive without a protocol change: root, holding the seed; reader, holding its device key and the envelope secrets, reading and seeding alone and recovering new envelopes from chain history; read-delegate, holding decrypted credentials in memory supplied by a root under delegation, reading while a root is reachable and never transferring, as a CI runner does; and signer, holding only its device key, approving purchases and signing intents within its permissions and never decrypting, as a phone does. The MVP declares root and reader. Revocation removes the device's signer from the contract account; the device's next attempt fails the wallet-control assertion and the conforming client purges its envelopes and envelope secrets, exactly as it purges a transferred entitlement. An offline device cannot pass the attempt rule once its view is older than `τ_soft`, so beyond that bound it holds only committed plaintext, and on reconnection one state read both loads new envelopes and purges moved or revoked ones. Transfer is gated by the device's signer permissions, reading by the envelope branch, and identity by the control branch.
...
None. Every decision carried in the technical approach, the dependency map, and the feasibility assessment is resolved above.
...
None. The multi-device model, the custody adapter's in-place upgrade, and the identity's chain-level form are stated under Resolved Positions.
...
- **The contracts are written against one sponsorship mechanism** and reworked when it is chosen. Mitigation: LC-13's signed-intent rule on every identity-bound mutation of the contract account.
- **A custody upgrade silently changes the principal.** Mitigation: the migration verifies re-derived public keys against the on-chain binding and envelope-key registration before retiring the old blob, and rolls back on mismatch.

# Stakeholder Communications

No external communication has occurred and none is planned before the demonstrable milestone. By audience:

- **Project lead.** This document and the revised business case; the stop criteria; the request to declare the latency budget and its conditions before the harness selects.
- **Implementers.** The technical approach, dependency map, and feature spec; the repository's agent process rules with their Rust examples; the foundation and harness tickets as the nodes to be authored; the Resolved Positions as their governing decisions.
