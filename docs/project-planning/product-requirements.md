...
- **The sidecar carries a wrapped piece-group key per set.** The encryptor chooses each piece-group key; each live parameter set's sidecar entry for a group carries that set's capsule and the key wrapped under a KDF of that set's encapsulated value, so independently generated sets open one ciphertext; a KEM output is never a piece-group key directly. A set made live after a deployment's registration reaches it by sidecar addition, authored by the asset's authority from an escrow-era credential, never by re-encryption; a credential opens only deployments carrying its set and the client selects accordingly. Recorded in cryptography.md under Credential KEM, Payload Encryption, and Escrow Key Lineage; LC-09 and AS-16 amended. External review finding applied at the user's direction, 2026-09-24.
- **The ingest attestation is the source's release signature, not the digest.** For npm, the registry signature over `name@version:integrity` against the registry's published keys; verified by the First Finder, on chain through Base's P-256 precompile against a governance-updated source-key table, and by every client against decrypted bytes. The registration lock is on the asset record; a later First Finder may register a further escrow deployment, so a deployment whose bytes fail attestation is dead rather than the identity poisoned. Recorded in cryptography.md, MVP Scope, FF-01, PC-02. Applied 2026-09-24.
- **The daemon does not distinguish same-user processes and does not claim to.** The package host confers resolution and serving only; every operation under local presence completes only after an interactive confirmation through a daemon-owned surface that no IPC message can satisfy. Recorded in MVP Scope's local-presence definition, XA-02, and the technical requirements' IPC surface. Applied 2026-09-24.
- **The npm entry point is `npx chaintorrent`**, one command, no install-time script. SI-02 and AS-02 amended. Applied 2026-09-24.
- **The outage guarantee is a pinned closure.** Package metadata is captured at every upstream resolution into an authenticated local store; tarball URLs stay on the canonical registry host so lockfiles are portable; the ledger keeps a per-name index of ingested versions for range resolution during an outage, with dist-tags best effort. Recorded as PR-09; AS-11 and AS-24 amended. Applied 2026-09-24.
- **CAS reuse is at the tarball.** Package managers extract per project; nothing links into the store; migration moves entries. ST-02 and the technical requirements' CAS layout amended. Applied 2026-09-24.
- **Chain-state quorum**: three configured nodes by default, two agreeing at a common reference within `τ_soft`; stale is not divergent; divergence and a view older than `τ_soft` fail closed. XA-06 and the architecture's trust boundary amended. Applied 2026-09-24.
- **Entitlement ownership changes only through delivery**; the token's standard transfer and approval entry points are disabled, callbacks run after state is final under a reentrancy guard. Recorded as LC-11. Applied 2026-09-24.
- **The relayer bounds exposure globally**: a per-window budget and maximum sponsored liability, per-identity limits as one layer, exhaustion an explicit failure. LC-10 amended. Applied 2026-09-24.
- **All non-functional requirements are now rows**: XA-08, XA-09, XA-10, IC-09, IC-10, RO-07, RO-08, LI-02. Applied 2026-09-24.
- **Plan structure corrections**: interface and type tickets folded into their first consumers, and the domain-types milestone dissolved; the sample deployment moved to the payload cipher milestone; the contracts milestone exits on Foundry proof and the chain adapters milestone carries LC-01; CD-05 and AS-26 exit at credential delivery; the relayer enters on the chain adapters. Recorded in the dependency map, milestones, and master plan. Applied 2026-09-24.
- **Success metrics carry three latency measurements**, whole command, artifact serving, and authorization attempt; the budget and its conditions are declared before the harness selects; an availability pilot runs at the dogfood baseline. Applied 2026-09-24.
- **The ledger is evidence of acquisition and ownership**, not an installed-software inventory. Revised business case amended. Applied 2026-09-24.
- **An install is never slower than the registry, and a first run ends in independence.** Plaintext comes from whichever source delivers it within the declared budget; an identity without a credential is served by the registry while it is available, since a grant is a chain transaction; the encrypted swarm path serves a first install only during an outage, with a bounded wait and an actionable failure that leaves the request pending. A durable grant request is registered for every unheld ledger-known asset whichever source served, batched per install into one sponsored operation, queued until submittable, and fulfillable while the requester is offline; the deployment's ciphertext is prefetched, pinned, and seeded meanwhile under the seeding settings and quota; after the grant lands the daemon matches the granted set to a held deployment. Branches: absent assets take the First Finder path and the finder grants itself; race losers request; a plaintext-root mismatch against the record marks the deployment dead and bootstraps a fresh one; explicit-publisher requests go to the issuance policy; priced assets become notices; held assets are skipped. Independence is of the registry and of other participants, and of everything while plaintext is retained; it is per asset version and visible per asset on every surface. The costs, first-run disk and bandwidth doubling and the closure published as requests at once, are consent items, and a cache-only mode with no chain identity exists. Recorded in MVP Scope under Package Manager Integration, the execution trace, PR-03, PR-08, PR-10 through PR-12, SI-19, SI-20, LC-10, LC-12, CD-05, RO-01, RO-03, AS-08, AS-29, and AS-30. User decision, 2026-09-24.
...
Before release, two internal checkpoints exist and neither is a release. The harness report, which closes the first delivery grouping with its integration test and commit and produces the measurements everything else waits on. The demonstrable milestone at the package host, where an ordinary `npm install` is served at the registry's speed, registers its requests and prefetches, resolves against the swarm with the registry down, and the north star, first-run independence, and the latency guardrail are first observed on the dogfood population.
...
- **The harness phase's tickets, under one file per turn, take longer than expected and no throughput baseline exists to say so early.** Mitigation: track tickets closed against the map from the first one; the independent starting points and the parallel KEM and envelope chains allow parallel authoring.
