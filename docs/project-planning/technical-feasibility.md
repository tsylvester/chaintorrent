...
| Wallets and account abstraction | EIP-1193, WalletConnect, EIP-712, ERC-4337 paymaster on Base | High | Standards exist; the identity's chain-level form and the sponsorship mechanism are chosen at the relayer milestone under LC-13 |
...
| Identity, wallet, and custody | Medium | The custody adapter's capability list, envelope keys in pairing groups, multi-device by paired transfer without principal change, and recovery from chain history; conventional under the local keystore with holder-seed derivation |
