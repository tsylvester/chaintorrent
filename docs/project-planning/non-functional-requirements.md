...
| NF-S18 | Device authority: the holder seed never leaves a root; every other device acts under its own key admitted on the identity's contract account; every attempt's wallet-control assertion is read in the same state view as the authorization, so a revoked device is denied at its next view and its conforming client purges | IW-05, EC-04, EC-08; product requirements, multi-device position | Stated |
...
| NF-M12 | Custody adapters upgrade in place: blobs headed by adapter version, wrapping-key identifier, and derivation parameters; a checkpointed migration verified against the on-chain binding and envelope-key registration before the old blob is retired | IW-02, IW-04; product requirements, default key custody | Stated |
