...
**Exit.** CD-07 and AS-21: sizes, timings, and delivery cost as L2 execution and L1 data fee on both curves; piece-group size and curve selected against the declared budget and recorded in release evidence; cross-set agreement among the vectors; the verifier deployed on Base Sepolia as shipped code; throughput recorded.
...
**Milestones.** Payload cipher and commitments; signature services and binding schema; registry and entitlement contracts; chain, settlement, and entitlement-state adapters; swarm transport and seed host, in parallel from the cipher milestone onward with no chain dependency.

**Entry.** Harness report closed; this grouping re-mapped to tickets.

**Exit.** CR-01, CR-02, CR-03, CR-06, with a two-set sidecar opening one ciphertext and the sample deployment bundled; LC-01 through LC-12 on Base Sepolia; XA-06 quorum views through the decision table; SW-01 through SW-07 with retrieval through the owned host and a delegated client and Bao rejection of a piece that passes SHA-1.
...
**Milestones.** Daemon skeleton with jobs, configuration, settings, resolver, health, telemetry, and IPC; identity and custody; plaintext CAS, resolution orchestrator, and package host; First Finder engine; the demonstrable milestone; credential delivery and per-attempt authorization.
...
**Yields.** A daemon that serves packages from whichever of cache, swarm, and registry delivers within budget, never slower than the registry, registers and prefetches in the background so a first run ends independent, bootstraps absent packages, and decrypts with nothing on the read path.
...
| Protocol core and contract suite | Payload cipher and commitments; signature services; registry and entitlement contracts; chain adapters; swarm transport and seed host | Unstarted; mapped as sprints |
| Local daemon and package serving | Daemon skeleton; identity and custody; CAS and package host; First Finder engine; demonstrable milestone; credential delivery and per-attempt authorization | Unstarted; mapped as epics |
...
- **Nothing depends on an undecided selection.** No node is authored against an open decision; the multi-device sync mechanism proceeds on its recorded recommendation, and every node that encrypts waits on the harness report for the piece-group size.
- **Independent milestones may run together.** The swarm transport runs beside the contract milestones; the relayer and the publisher path run beside the installer; the demonstration harness runs beside the daemon grouping. The map's edges decide, not a calendar.
- **External work starts at the harness.** Cryptographic review, legal drafting and review, signing enrollment, and the platform-matrix consequences all begin when the harness grouping begins, so they gate nothing late.
...
- **Resolution decays outward.** Only the grouping in progress is at ticket resolution; the next is re-mapped when the current one closes; the plan and the map are revised in place with no history carried.
...
- **No tickets beyond the grouping in progress.** Detail for distant groupings is authored when they are re-mapped, never before.
...
| --- | --- | --- |
| `domain` | Types and guards | Workspace and discipline bootstrap for the skeleton; every milestone thereafter |
| `adapters/pairing`, `adapters/kdf` | Pairing adapters, KDF, hash-to-scalar | Pairing adapters and key derivation |
| `adapters/kem` | Credential KEM | Credential KEM |
| `adapters/envelope` | Envelope | Envelope |
| `adapters/proof`, `contracts`, `adapters/chain/verifier_client` | Delivery proof, Solidity verifier, verifier client | Delivery proof and Solidity verifier |
| `apps/harness-crypto` | Vectors, measurement, report | Harness vectors, measurement, and report |
| `adapters/cipher`, `adapters/hashing` | Payload cipher, Bao commitments, validation | Payload cipher and commitments |
...
| `adapters/transport`, `adapters/discovery`, `adapters/seed_host` | Swarm engine, ciphertext store | Swarm transport and seed host |
| `adapters/jobs`, `adapters/config`, `workflows/settings`, `workflows/compose`, `workflows/health`, `adapters/telemetry`, `adapters/ipc`, `apps/daemon` | Daemon skeleton | Daemon skeleton |
| `adapters/custody`, `workflows/identity` | Custody, identity manager, secret region | Identity and custody |
| `adapters/cas`, `workflows/resolve`, `adapters/package_host_npm` | CAS, orchestrator, package host | Plaintext CAS, resolution orchestrator, and package host |
| `adapters/ingest_npm`, `workflows/first_finder`, `workflows/grant` | First Finder engine, grant service | First Finder engine |
...
| `workflows/publish`, `adapters/identity_proof` | Publisher engine, issuance policy, proof adapters | Explicit publisher path and issuance policy |
...
The rings, the adapter table with MVP implementations and declared capabilities, the host shells, and the daemon's twenty-one subsystems with trust boundaries, process model, and storage ownership. In the system architecture; crate and milestone assignment in Component Mapping above.

# Integration Points

Fourteen boundaries an integration test must cross, from verifier parity to the site demonstration, each crossed in the milestone that first joins its two sides. In the system architecture and the milestones' Integration Requirements.
