...
- **Remote encrypted-hit rate.** Installs served from the swarm rather than upstream, which on the ordinary path happens for holders and during registry outages. Growth means the swarm carries what developers need (PR-03, AS-08).
- **Grant-fulfilment latency.** Time from request registration to credential loaded, measured separately from swarm health, since a swarm can hold every byte while no holder is online to grant (PR-10, CD-05; AS-29).
