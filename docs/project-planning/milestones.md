...
| --- | --- | --- |
| Cargo workspace with rings as crates | A domain crate, a workflows crate, one crate per adapter family, apps, each created by the node of the first module that lives in it; glob members; a lint table forbidding `unsafe_code` and denying `unwrap_used`, `expect_used`, `panic`, and `as_conversions` in production code; a ring violation is a compile error | Workspace and discipline bootstrap for the manifest; each crate at its first module |
| Shared test support | The dev-only `test-support` crate with `assert_same`, a dev-dependency of every crate | Workspace and discipline bootstrap |
| Continuous integration on clean ephemeral runners for Windows, macOS, and Linux | `cargo check`, `cargo clippy`, `cargo fmt --check`, the tests, `cargo-audit`, `cargo-deny` on every push; `forge build` and `forge fmt --check`, the TypeScript linter, `cargo-fuzz`, and end-to-end scenarios on clean runners, each added by the ticket that first needs it | Workspace and discipline bootstrap for the definition; `contracts/PairingLib` for `forge`; the first shell or webview ticket for the TypeScript linter; the first fuzz target and the first end-to-end scenario for theirs |
...
| Canonical encoding adapters | `IEncoderAdapter` and `IDecoderAdapter`, sharing one versioned encoding identifier, for everything hashed, signed, stored, or framed over IPC; implemented in the MVP as Ethereum ABI encoding through `alloy`'s sol types, which Solidity reproduces natively, with the adapters owning the sol-type mirrors and conversions | Workspace and discipline bootstrap |
| Secret type | A `domain` type that cannot be formatted or serialized, exposes the wrapped value only through an explicit accessor, and zeroizes on drop | Workspace and discipline bootstrap |
| Tracing with correlation | Per-request correlation identifiers across every subsystem | Workspace and discipline bootstrap |
...
**Scope.** The tickets `workspace/cargo`, `test-support/assert_same`, `domain/secret`, `encoding/abi_encode`, `encoding/abi_decode`, `telemetry/tracing`, and `workspace/ci`: the workspace manifest with glob members and the lint table; `rust-toolchain.toml`, `deny.toml` with the license allowlist; the `test-support` crate; the `domain` crate, created by its secret module; the `adapters/encoding` crate with the `IEncoderAdapter` and `IDecoderAdapter` interfaces sharing one versioned encoding identifier, the ABI encoder and decoder, and the sol-type mirrors they own; the `adapters/telemetry` crate with tracing setup and per-request correlation; no crate created, dependency pinned, or tool configured ahead of the ticket that first needs it; the element mapping applied as one module directory per function with one file per element; the GitHub Actions matrix over Windows, macOS, and Linux running the `cargo` checks, the tests, and the audits; Apple, Microsoft, and Sigstore signing enrollment started. The manifests and the CI definition are configuration files with no types and no tests.

**Entry.** None.

**Exit.** The workspace and every crate its tickets create build and pass checks on Windows, macOS, and Linux; the lint table rejects the denied constructs in production code; `cargo-deny` enforces the allowlist; the secret type is proven at compile time to implement no formatting or serialization trait; the ABI encoder and decoder round-trip their known-answer vectors and the decoder rejects malformed input. Requirement rows: CR-07 for the secret type, RO-04 for correlation, XA-07 for the facilities, NF-M07 as the authoring discipline.

**Deliverables.** The workspace; the CI definition; the test-support, domain, encoding, and telemetry crates; the signing enrollment requests filed.
