...
**Scope.** `adapters/custody/local_keystore` over `keyring` with a wrapping key and encrypted blob; `holder_seed` derivation of envelope secrets, handshake key, and the local chain key; `device_roles` declaring only the full role with the versioned capability set; `workflows/identity` create or import, relayer-paid binding through the chain adapter, envelope-key registration with proofs of possession, EIP-1193 and WalletConnect signing-only connections, paired local transfer as the declared multi-device mechanism; the persistent credential store per entitlement; the secret region types.

**Entry.** Daemon skeleton; signature services; chain adapters, for binding and registration on Base Sepolia.

**Exit.** IW-01 through IW-07: distinct schemes composed and surviving restart; capability combinations failing closed; one identity, one binding, one registration across restart and reinstall; no decrypted credential or key in any custody surface; recovery on a second device profile by seed transfer; wallets signing only; envelope keys never wallet keys.
...
**Scope.** `apps/cli` with `clap`: consent, settings, diagnostics, package-host control, identity, jobs, repair, uninstall; `apps/desktop` on Tauri 2: configuration, identity and custody workflows including paired seed transfer, adapter selection, jobs, the seeding archive, the settings catalogue, diagnostics, repair, recovery.
