...
Draft, 2026-09-23. An overview of how the MVP is built, drawn from [cryptography.md](../research/cryptography.md), [MVP Scope](../research/MVP%20Scope.md), [MVP Application Requirements](../research/MVP%20Application%20Requirements.md), [MVP Execution Trace](../research/MVP%20Execution%20Trace.md), and the To-Do list and proposed build sequence in the [ChainTorrent MVP workplan](../workplans/current/ChainTorrent%20MVP.md). Those documents are authoritative; this one arranges them for a reader deciding how to build, and it adds no rule they do not already carry. Open questions are listed exactly as the workplan holds them, with a Feedback block after each for the user to answer; a blank answer means the build proceeds on the assumption stated there.
...
| `IPairingAdapter` | `Bn254PairingAdapter` or `Bls12381PairingAdapter` | Chain adapter, from the launch network |
...
| `IIngestSourceAdapter` | npm registry, as-is tarball with integrity attestation | Policy: free-to-use archives only |
| `IPackageHostAdapter` | npm registry protocol served locally | Package manager |
| `ISwarmTransportAdapter` | Owned implementation plus `BitTorrentAdapter` compatibility | Deployment, multi-homed |
...
| `IKeyCustodyAdapter` | Unresolved blocking selection; default identity is a local key | Installation |

## Cryptographic construction, stated once

Type-3 pairing groups of prime order; curve BN254 or BLS12-381 as the launch chain's precompiles dictate. A parameter set is `g1, u0, u1` in the first group and `g2, hpub = g2^α` in the second, with master scalar `α` held by the issuer. The entitlement's identity element is `F_I = u0 · u1^I`. A credential is `(g1^α · F_I^r, g2^r)`; the seller rerandomizes it as `(A · F_I^s, B · g2^s)` without the master scalar; validity is the public pairing check. A capsule is `(g2^t, u0^t, u1^t)`, well-formed under two pairing checks, encapsulating `e(g1, hpub)^t`; decapsulation is two pairings and one scalar multiplication. The piece-group key is a domain-separated KDF of that value and the context. Envelopes are ElGamal in each source group to the recipient's two independently keyed envelope keys. Delivery proofs are generalized Schnorr under Fiat–Shamir over the full statement, six scalars, plus three second-group elements where the verifier has only first-group arithmetic. Security rests on SXDH, decisional BDH-3b, and the random-oracle model. Under the escrow suite the identity element is fixed per asset, the capsule shrinks to two elements, and any current holder can author a grant's credential.
...
| npm bootstrap package | Minimal JavaScript shim over signed Rust artifacts | `npm install chaintorrent` as a complete alternative onboarding path invoking the same installer |
...
| Relayer or paymaster | Rust service plus ERC-4337 support where available | Sponsors the free path and the one-time identity binding; rate limits; cost reporting |
...
- **Package serving and resolution** (PR): npm-compatible host leaving resolution to npm; deterministic coordinate-to-identity mapping; fixed resolution order; verified atomic plaintext CAS; cache policy separating plaintext and ciphertext lifecycles; plaintext hits with no remote access; ciphertext through authorization before serving; upstream bytes served without waiting for bootstrap.
...
- **Cryptographic services** (CR): payload cipher, Bao commitments and challenges, signatures, envelopes, randomness and derivation lineages, descriptor validation, secret lifecycle, credential KEM, delivery proof, pairing adapter.
...
- **Header sidecar**: one per live parameter set, the deployment's common public capsules, one per piece group, committed by its own root; useless without a credential; seeded like any object.
...
| --- | --- | --- | --- |
| Plaintext CAS | Client | Content hash | Persists by design; outside the authorization boundary; user quota with pinning of linked artifacts and warning before evicting anything not re-authorizable |
...
**Target platforms.** Supported operating systems and architectures for the daemon, CLI, desktop application, and extension, with signed native artifacts per platform. The packaged MVP runs without Rust, Solidity tooling, or repository source; npm itself is the only developer tool assumed.

**Installation.** Both shells invoke one Rust installation coordinator executing a durable plan: detect platform and capabilities; download and authenticate artifacts; create and permission stores and the IPC endpoint; install, start, and enable the daemon or an equivalent persistent user service; offer explicit reversible package-manager redirect; create or import identity and complete the relayer-paid binding and envelope-key registration; configure default chain, discovery, relay, and ingest endpoints; resolve the full adapter composition; and report ready only on an active end-to-end health probe. Every mutating stage is backed up and rolls back on later failure.

**Chain deployment.** The contract suite deploys to the chosen Ethereum network through an on-chain factory that injects constructor-bound adapter addresses from a governance-controlled adapter registry, immutable once bound, with optional EIP-1967 proxies for adapter logic. The validation harness deploys the verifier to a test chain first. Delivery verification uses EIP-196 and EIP-197 on BN254 everywhere, and EIP-2537 on BLS12-381 where deployed; per-launch-network availability of the newer precompiles must be confirmed.
...
The workplan holds a proposed build sequence, derived from dependency rather than interest, and records that no node has been authored against it. It is adopted here as the sequencing and remains a proposal until ratified.

1. **Decisions before nodes.** The credential construction, key custody approach, host adapter approach, and claim granularity are settled; the launch network is Base and the escrow record carries no maintainer commitment. The license is not settled and is a release prerequisite rather than a node blocker.
2. **Cryptographic validation harness.** KEM, envelope, and delivery proof on the resolved pairing adapter with the verifier deployed to a test chain, producing the measurements that fix the piece-group size. It runs on BN254 until the network is chosen and precedes any node that encrypts, because piece geometry is a suite parameter every later deployment carries.
3. **Swarm transport, seed host, and the ciphertext store.** Provable end to end with no chain and no cryptography: seed an object and its sidecar, fetch them elsewhere, verify Bao paths.
4. **Registry contract.** Batch resolve, batch authorize, pagination, envelope-key registration, the parameter-set registry with the sidecar-coverage rule, per-entitlement interval state, delivery verification through the precompiles, and the per-package-and-version claim-set state layout.
5. **First Finder ingest.** Fetch, verify attestation, generate the parameter set, encrypt per group, build the sidecar, register, seed. This is the spine both publisher paths reuse.
6. **Package host adapter.** At which point an ordinary `npm install` resolves against the swarm and the MVP has something to demonstrate.
7. **Credential delivery and per-attempt authorization.** Mint delivery through the relayer, local decryption under the attempt rule, interval-end destruction, closing the read path.
8. **Explicit publisher path** as branches off First Finder, then **transaction flow proof**, where transfer delivery is first exercised, then **escrow claim** last, since nothing else depends on it.

Within every node, the workplan's fixed element order applies: interface test, interface, interaction spec, mock, guard test, guard, unit tests, construction, implementation, provides, integration test, directionality, requirements, commit. Nodes are authored bottom-up in dependency order and one file is edited per turn. The onboarding shells, desktop application, CLI, relayer, claim verifier, seed host, demonstration harness, and observability land alongside the steps they first serve rather than as a separate phase, because readiness in step 6 already requires installation, health, and the local daemon.
...
| Settlement reversal | Reads at `SOFT`, destruction at `HARD`; the exposure is one entitlement's credential, bounded and self-healing; tier declared per deployment |
...
| A single RPC node censoring reads | The per-attempt state view is obtainable from more than one configured node; one unreachable node does not deny |
...
Listed as the workplan holds them, grouped by what releases each. Items that release with the stake and token layer or with a later content class are outside the MVP build and are summarized; items on the MVP's path carry a Feedback block. A blank Feedback block means the build proceeds on the stated assumption.

## On the MVP path

**Launch network and pairing curve.** Resolved: Base, with Base Sepolia as the test network; BLS12-381 primary through EIP-2537 and BN254 retained; tiers mapped to Base; delivery cost measured as L2 execution and L1 data fee. Recorded in MVP Scope's Entitlement Ledger section and the product requirements.

**Default key-custody implementation.** Resolved: a local keystore under the OS credential store as the first `IKeyCustodyAdapter`, keys derived from a holder seed, device roles as versioned custody capabilities with only the full role in the MVP, external wallets signing only. Recorded in the product requirements.

**Escrow-salt custodian.** Resolved: none; no maintainer commitment is stored, the verifier establishes the claim set from upstream metadata at verification time, and a commitment is reopened only with a ZK-Email verifier. Recorded in MVP Scope's Escrow Claim section and the product requirements.

**Attempt-rule parameters and piece-group size.** Resolved: chosen from harness measurement against the declared latency budget. Recorded in the product requirements.

**Distribution and client license.** Resolved: build proceeds; the license text is a release prerequisite; the content-terms field is a string supplied at publication. Recorded in the product requirements.

**Scope statement of the authorization invariants.** Resolved: left to the specification's next revision. Recorded in the product requirements.

**Build sequence ratification.** Resolved: the proposed order is ratified and the first node is a harness ticket. Recorded in the product requirements.

## Held behind a policy line

**Ingest adapter eligibility.** Adapters target archives already free to use. An adapter aimed at licensed content waits on the post-claim pricing paradox; one aimed at private or internal content waits on consumption privacy. Crossing either is a recorded policy decision, not an engineering convenience. No MVP build impact.
...
**Relationship to the other planning documents.** The [business case](business-case.md) groups this sequence into five coarser phases; where the two differ, this document's sequencing, taken from the workplan, is the finer and more recent statement. The [feature spec](feature-spec.md) maps each module family here to a feature with acceptance criteria, and the [success metrics](success-metrics.md) define what the harness and the acceptance run must produce.

**Language and runtime policy, restated for the builder.** All deployable applications are Rust; GUI surfaces use Tauri; headless services and the CLI are Rust without Tauri. Protocol rules, workflows, cryptography, storage, networking, credential delivery, and adapter contracts are shared Rust libraries so every surface executes the same behavior. TypeScript exists only where Visual Studio Code and npm require it and owns no protocol rule, cryptographic decision, entitlement logic, resolution, or workflow state. Solidity exists only because the EVM requires bytecode.

**Proof model.** Every requirement is simultaneously an operational requirement and an integration or end-to-end test obligation. Passing isolated unit tests does not satisfy a requirement whose proof crosses a process, adapter, storage, network, custody, or chain boundary. Completion is all acceptance scenarios passing through the packaged applications on clean machines with no test-only bypass of onboarding, the package host, capability resolution, or the deployed verifier.

**What is not decided here.** This document proposes nothing beyond what the specifications and workplan already carry. Where it states an assumption under an open question, that assumption is the builder's default in the absence of an answer and is not a decision.
