...
6. **Package host adapter.** At which point an ordinary `npm install` is served at npm's speed, registers its requests, and prefetches its ciphertext, and the MVP has something to demonstrate.
7. **Credential delivery and per-attempt authorization.** Mint delivery through the relayer, request fulfilment by holders for requesters present or absent, local decryption under the attempt rule, interval-end destruction, closing the read path and completing a first run's independence.
