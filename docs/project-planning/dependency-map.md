...
| Relayer or paymaster | Free-path sponsorship under a global per-window budget and maximum liability with per-identity limits as one layer; the sponsorship mechanism for the identity contract account chosen here under LC-13; sponsored device admission and revocation; cost and budget reporting | chain adapters, so it may start before the daemon grouping closes |
...
- **The identity is a contract account; its sponsorship mechanism is chosen at the relayer milestone.** The contract sprint builds the account with its device registry and takes every identity-bound mutation as a signed intent under LC-13, so a paymaster, a relayer, and a self-funded caller all remain open.
