...
* **Decisions before nodes.** The credential construction, key custody, host adapter approach, claim granularity, launch network and curve, and the absence of a maintainer commitment in the escrow record are settled; the identity's chain-level form waits for the relayer milestone under the contracts' signed-intent rule; the license is not settled and is a release prerequisite rather than a node blocker.
