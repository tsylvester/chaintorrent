...
After every edit, the touched file is linted clean, and completion is proven — never assumed.
...
When a test is written before its interface, guard, or implementation, the compiler and linter will report unresolved errors — that is the **RED proof** the test defines a contract nothing yet satisfies (see [tests](tests.md), [tdd-ordering](tdd-ordering.md)).
