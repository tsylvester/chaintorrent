...
An attempt is one piece-key derivation from one capsule. Before it, the conforming client holds a state view at the deployment's declared tier, no older than `τ_soft`, showing the wallet as owner at the credential's interval, a wallet-control assertion renewed every `τ_wallet`, and the authenticated payload descriptor. On a transfer out it stops at the declared tier and destroys the credential only at HARD, so a reorganized transfer never strands an owner; on a transfer in it uses the credential once settlement is at the declared tier and the tuple passes the local validity check. No attempts offline, which matches the user's stated always-online client. The persistent credential is `(x, y, E_o, interval index)`, constant size; the reading tuple is decrypted from `E_o` at start-up and may be rerandomized in memory, and a sale always starts from the fresh decryption of `E_o`. Losing `(x, y)` ends the ability to transfer (P13). `τ_soft` and `τ_wallet` are implementation parameters against the chain adapter's tiers (Q05 section). Adoption into the specifications is pending and separate.
...
- **Payload.** The encryptor chooses a piece-group key per group; under each live set the sidecar entry carries that set's capsule and the key wrapped by XOR under a domain-separated KDF of `K` and the context (asset, deployment, suite, parameter set, group index, geometry); each group encrypted once. Everyone receives the same encrypted pieces and the same sidecars, each set's sidecar its own object; a body carries one per live parameter set.
...
- **Costs, by formula.** Per capsule three source-group elements under the entitlement scope and two under the asset scope, 128 bytes compressed on BN254 and 192 on BLS12-381 for three, plus 32 bytes of wrapped piece-group key per group per live set; per interval on chain a four-element envelope and a six-scalar proof, plus three `G2` elements on BN254; per holder a constant credential. For a 16 KiB piece group with one live set that is between roughly 0.8 and 1.4 percent of payload depending on curve and scope, and a few hundredths of a percent for 1 MiB; each further live set adds its own sidecar. No byte, latency, proof or settlement cost has been measured.
